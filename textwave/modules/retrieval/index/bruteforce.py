...
class FaissBruteForce:
    ...
    def search(self, query_vector, k=5):
        """
        Searches the index for the top-k nearest neighbors of the given query vector.
        ...
        Returns:
            list of dicts: Each containing 'score' and 'metadata' for a match.
        """
        vector = np.array(query_vector).astype(np.float32).reshape(1, -1)
        if self.metric == "cosine":
            faiss.normalize_L2(vector)
